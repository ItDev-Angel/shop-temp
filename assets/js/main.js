...
const basketBtnOrder = document.querySelector('.basket__btn-order');
let cardCounter = 0;
...
const btnClickHandler = (e) => {
  ...
  if (target && target.matches('.item-actions__cart')) {
    ...
    disableControls(target, btnClickHandler);
      setTimeout(() => { target.innerHTML = restoreHTML;
...
const createBasketOrder = () => {
  orderBasket();
  const basketBtnOrder = document.querySelector('.basket__btn-order');
  basketBtnOrder.addEventListener('click', orderClickHandler);
};
